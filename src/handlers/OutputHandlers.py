...
from src.MessagesFile import DEAD_MESSAGE, GREETING_MESSAGE, HELP_MESSAGE, PICTURE_MESSAGE, NEED_TO_REGISTER_MESSAGE
...
async def today_schedule(message: aiogram.types.Message):
    if not db.is_registered(message.from_user.id):
        await message.answer(NEED_TO_REGISTER_MESSAGE)
    else:
...
@dp.message(lambda message: message.text.lower() == 'пикрандом' or message.text.lower() == '/picture')
async def pic_random(message: aiogram.types.Message):
    if not db.is_registered(message.from_user.id):
        await message.answer(NEED_TO_REGISTER_MESSAGE)
    else:
        await telebot.send_photo(message.chat.id, picture_choose(), caption=PICTURE_MESSAGE)
